...
ne_countries(scale = "medium", returnclass = "sf") %>%
  ...
  st_transform(crs = "+proj=eqearth +wktext")-> world

# figure out the center of Maine
ca <- which(state.abb == "CA")

st_sfc(st_point(c(state.center$x[ca], state.center$y[ca]))) %>%
  st_set_crs("+proj=longlat +datum=WGS84 +no_defs") %>%
  st_transform(crs = "+proj=eqearth +wktext") -> ca_center # need this in the eqearth CRS (see note below)
...
# table 1 
tst <- html_nodes(pg, "table") %>%  # parse the page
  .[[2]] %>%
  html_table() %>%
  as_tibble() %>%
  janitor::clean_names() %>%
  ...
  mutate(ca = ca_center) -> xdf # temporary variable
...
ggplot() +
  ...
  geom_curve(
    data = ca_exports_to, aes(x = from_x, y = from_y, xend = to_x,
                              yend = to_y, color = x2018_value, 
                              size = x2018_value),
    curvature = 0.2, arrow = arrow(length = unit(10, "pt"), 
                                   type = "closed"),
  ) +
  guides(
    color = guide_legend(reverse = TRUE)
  ) +
  annotation_custom(tabl, xmin = -16920800, xmax = -13800000,  ymin=761200/2.2, ymax = 761200) + # values are in eqarea meters
  coord_sf(crs = "+proj=eqearth +wktext") +
  scale_color_distiller(
    palette = "RdYlBu", trans = "log10", name = "(Size & color\nlog10 scale)", label = scales::comma,
    breaks = c(10000, 20000, 30000, 40000), limits = c(1000, 40000)
  ) +
  ...
  labs(
    x = NULL, y = NULL,
    title = "        Top 20 (by Value) Export Destinations for CA Goods (2018)",
    caption = "Data source: Census Bureau Foreign Trade <https://www.census.gov/foreign-trade/statistics/state/data/me.html>\nbased on code by hrbrmstr/y2019-30daymapchallenge • #30DayMapChallenge"
  ) +
  theme_ft_rc(base_size = 8, plot_title_size = 10, grid="") +
  theme(legend.key.height = unit(2, "lines")) +
  theme(legend.position = c(0.95, 0.8)) +
  theme(axis.text.x = element_blank())


# Value of Specific Exports -----------------------------------------------
